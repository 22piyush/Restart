...
. only visible element are included

-Layout
Browser Calculate : position , size, box model
...
-Composite
. Layers are combined and sent to GPU (Graphics Process Unit) then final screen rendering happens.
. Browser draw each layer seperately then combine them into final screen.
. Not everything is draw together some elements are promoted to seperate layers to improve performance.
i.e  the animation are work smooth, when we change css no need to recalculate layout, no repaint, browser doesnt repaint everything it just composite layers.
...
Lifting State Up
==================
-Moving state from child components to a common parent, and then passing:
-data (state) via props functions (handlers)
 via props to child components.
--Why we do this
-Multiple child components need the same data We want a single source of truth Child components stay simple & reusable
